...
export async function generatePdf (profile, reasons, pdfBase) {
  ...
  const pdfCreationInstant = new Date()
  const pdfCreationDate = pdfCreationInstant.toLocaleDateString('fr-CA')
  const pdfCreationHour = pdfCreationInstant.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }).replace(':', '-')
  downloadBlob( new Blob([pdfBytes], { type: 'application/pdf' }), `attestation-stime-${profile.heuresortie}_${pdfCreationDate}_${pdfCreationHour}.pdf`)
}
